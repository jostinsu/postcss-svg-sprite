class CSS {
	...
	constructor(shapes, opt) {
		...
		this.rulesInfo = [];
		this.commonRuleInfo = {
			className: `.${opt.nameSpace}${this.block}`,
			attr: {
	...
	getCss() {
		const commonCssRule = CSS._makeCssRule(this.commonRuleInfo);
		const shapesCssRule = this._makeShapesCssRule();
	...
	_getRuleInfoByShape(shape) {
		...
		return {
			className: `.${this.nameSpace}${this.block}${this.separator}${shape.name}`,
			attr: {
