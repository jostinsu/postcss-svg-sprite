...
class Sprite {
	...
	constructor(svgs, options) {
		...
		this.spriteContents = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg"></svg>';
	...
	isSvgsChange() {
		...
		_.sortBy(this.svgs, ['path']).forEach(svg => {
			hash.update(svg.id);
		});
		this.spriteId = hash.digest('hex');
	...
	_getSpriteInfo() {
		try {
			const spriteDom = new DOMParser().parseFromString(fs.readFileSync(this.spritePath, 'UTF-8'), 'image/svg-xml');
			return {
	...
	_updateShapeDom() {
		this.shapes.forEach((shape, index) => {
			const shapeLayoutInfo = this.layout.items[index];
	...
	getShapesFromSvgs() {
		...
		return Sprite.formatSvg(svgs, this.svgDir).then(svgs => {
			svgs.forEach(svg => {
				const doc = new DOMParser().parseFromString(svg.file.contents.toString(), 'image/svg-xml');
				shapes.push({
			...
			this.shapes = _.sortBy(shapes, ['name']);
	...
	getShapesFromSprite() {
		try {
			const spriteDoc = new DOMParser().parseFromString(fs.readFileSync(this.spritePath, 'UTF-8'), 'image/svg-xml'),
				svgDoms = spriteDoc.documentElement.getElementsByTagName('svg');

			return Array.from(svgDoms, dom => {
				return {
					dom: dom,
	...
	static layout(shapes, spacing) {
		...
		shapes.forEach(shape => {
			const width = parseInt(shape.attr.width) + spacing;
			const height = parseInt(shape.attr.height) + spacing;
